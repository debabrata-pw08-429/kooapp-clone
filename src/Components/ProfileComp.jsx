...
import {Card,CardBody,Image,Stack,Heading,Text,Button,Box,Spinner} from "@chakra-ui/react";
  ...
  return userFollowState == true ? (
    <div>
        <Card alignItems='center'  height={'12em'} width={'130px'} >
            ...
            <Button border='1px solid #4b4b4b' onClick={changeFollow} variant='solid' bg={bt1===0? '#4b4b4b' : "#ffffff"} colorScheme='#4b4b4b' borderRadius='20px' position='relative' bottom='-10px' height={'26px'} width={'79px'} fontSize= '12px'>{bt1 === 0 ? bt : bt1 === 1? <TiTick fill={'#4b4b4b'} size={'20px'}/>: ''} {showSpinner && <Spinner />} </Button>
        </Card>
    </div>
  ):(
    <></>
